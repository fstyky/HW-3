import json
from .models import UProfile, Room, Message
from channels.generic.websocket import WebsocketConsumer
from asgiref.sync import async_to_sync


def ulist():
    objects = UProfile.objects.filter().order_by('-name')
...
class WSConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        if 'delete_user' in message:
            ...
            user = UProfile.objects.get(id=id)
            user.delete()
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        if 'usersendcommandroom' in message:
            ...
            if message['usersendcommandroom'] == 'message':
                ...
                username = UProfile.objects.get(id=user_id).name
                message = message['message']
                message_save = Message(author=UProfile.objects.get(id=user_id), room=Room.objects.get(id=room_id), text=message)
                message_save.save()
